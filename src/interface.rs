...
impl Interface {
    ...
    pub fn play_ai(&mut self, diff: isize) -> Result<(), String> {
        ...
        let state = state.cond_flip(team).choose_next(diff).map(|x| x.cond_flip(team));
    ...
    pub fn play_at(&mut self, x: u8, y: u8) -> Result<(), String> { 
        let team = self.player_id();
        self.history.push(self.get_last().play_at_team(team, x, y).ok_or("it's full! :(".to_string())?);
        Ok(())
    }
